...
これができてしまうと、cookieの値を盗むことができる。cookieが盗めるということは、一時的にでも成り済ましをすることができるということ。
...
一番シンプルな共通鍵暗号は、古典的だけが、アルファベットの順番をずらすというもの。たとえばA->DにB->Eにという具合に。

これはいくつずらすのかという値が鍵になる。だが、ずらす幅は26通りしかないので、すぐにわかってしまうし、英語だとわかっていれば一番発生しやすい文字がEであることから、何文字ずらしているかすぐにわかる。

次にシンプルなものはXORを使うもの。
http://ja.wikipedia.org/wiki/排他的論理和

たとえば、10110111という情報を01010101という鍵でそれぞれXORをとり、もう一度同じ鍵でXORをとったらどうなるか。たしかめてみよう。

現在は、これらをさらに発展させて[AES](http://ja.wikipedia.org/wiki/Advanced_Encryption_Standard)という暗号を使っている。
...
実際には、nやe,dはきわめて大きな数であり、巨大な数の因数分解が現在のコンピュータでは途方もなく時間がかかることが、安全性の担保になっている。

## 一方向関数
http://ja.wikipedia.org/wiki/暗号学的ハッシュ関数

### 課題
1. [エ]1万個のランダムで異なる文字列にsha-1処理を行い、すべて値が違っていることを確認しよう。
